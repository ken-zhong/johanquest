...
import { canvasWidth, canvasHeight, checkCollision } from './util';
...
class Game {
  ...
  cleanUp() {
    // THIS DOES NOT WORK. WEBPACK WON'T COMPILE. WHY?
    for (int i = 0; i < this.exxons.length; i++) {
      if (checkCollision(this.exxons[i], this.johan) == true) {
        this.exxons[i].deathFlag = 1;
      }
    }

    /*  // THIS VERSION WORKS FOR SOME REASON.
    if (this.exxons.length > 0) {
      if (checkCollision(this.exxons[0], this.johan) == true) {
        this.exxons[0].deathFlag = 1;
      }
    }*/
